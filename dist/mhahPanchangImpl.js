...
class MhahPanchangImpl {
    ...
    daysInMonth(m, y) {
        let g_days = [31, 28, 31, 30, 31, 30, 31, 31, 30, 31, 30, 31];
    ...
    moon(jd) {
        ...
        for (let i = 0; i < 93; i++) {
            // outrage at length
            let arg = this.mhahMoonOutragConst.corrMoon[i].mlcor * ml +
            ...
            if (this.mhahMoonOutragConst.corrMoon[i].mscor !== 0) {
                sinarg *= i1corr;
                if (this.mhahMoonOutragConst.corrMoon[i].mscor === 2 ||
                    this.mhahMoonOutragConst.corrMoon[i].mscor === -2)
                    sinarg *= i1corr;
            }
            if (this.mhahMoonOutragConst.corrMoon[i].fcor !== 0)
                sinarg *= i2corr;
            lk += this.mhahMoonOutragConst.corrMoon[i].lcor * sinarg;
        }
        for (let i = 0; i < 27; i++) {
            // outrage at length additional
            let arg = this.mhahMoonOutragConst.corrMoon2[i].ml * ml +
    ...
    calcayan(jd) {
        ...
        let aya = 17.23 * Math.sin(this.d2r * om) +
